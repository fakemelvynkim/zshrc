# Configuraiton
    alias cdz='cd ${ZSH_HOME_DIRECTORY}'
    alias cdv='cd ${VIM_HOME_DIRECTORY}'

# vim-related
    alias v='vim'
    alias vi='vim'

# sync folder
    alias cds='cd ${SHARED_DIRECTORY}'
    alias cdsg='cd ${PUBLIC_GOOGLE_DRIVE_HOME_DIRECTORY}'
    alias cdsgg='cd ${PRIVATE_GOOGLE_DRIVE_HOME_DIRECTORY}'
    alias cdsd='cd ${PUBLIC_DROPBOX_HOME_DIRECTORY}'
    alias cdsdd='cd ${PRIVATE_DROPBOX_HOME_DIRECTORY}'

# codes
    alias cdc='cd ${PRIVATE_GOOGLE_DRIVE}/code'
    alias cdcapi='cd ${PRIVATE_GOOGLE_DRIVE}/code/api'
    alias cdca='cd ${PRIVATE_GOOGLE_DRIVE}/code/android'
# codes.android
    alias cdcae='cd ${PRIVATE_GOOGLE_DRIVE}/code/android/example'
    alias cdcal='cd ${PRIVATE_GOOGLE_DRIVE}/code/android/lib/'
    alias cdcalu='cd ${PRIVATE_GOOGLE_DRIVE}/code/android/lib/ui/'
    alias cdcald='cd ${PRIVATE_GOOGLE_DRIVE}/code/android/lib/database/'

# music
    alias cdms='cd ${PUBLIC_GOOGLE_DRIVE_HOME_DIRECTORY}/entertain/spotify'
    alias cdmp='cd ${PUBLIC_GOOGLE_DRIVE_HOME_DIRECTORY}/entertain/pandora'
    alias cdmy='cd ${PUBLIC_GOOGLE_DRIVE_HOME_DIRECTORY}/entertain/youtube'

# temp
    alias cdt='cd ${PRIVATE_GOOGLE_DRIVE_HOME_DIRECTORY}/temp'
